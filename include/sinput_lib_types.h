...
{
    SINPUT_CONFIG_OK = 0,
    /** Device config contains error params that have been overridden */
    SINPUT_CONFIG_ISSUES_OVERRIDE,
...
/** @brief High-level gamepad identity class (aligned with SDL-style labels). */
typedef enum
{
    ...
    SINPUT_SDL_GAMEPAD_TYPE_STEAM, 
    SINPUT_SDL_GAMEPAD_TYPE_COUNT
} sinput_sdl_gamepad_type_t;

/** @brief Physical layout of face buttons for feature / host hints. */
typedef enum
{
    ...
    SINPUT_SDL_GAMEPAD_FACE_STYLE_COUNT
} sinput_sdl_face_style_t;

/** @brief Describes whether controls are on a separate pad or integrated handheld. */
typedef enum
{
...
typedef struct
{
    ...
    struct
    {
        ...
        bool triggers;     // Left and Right DIGITAL triggers (ZL/ZR etc)
...
/** @brief Optional left/right touch surface coordinates and pressure. */
typedef struct
{
    struct
    {
